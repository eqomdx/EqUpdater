"""
EqUpdater - a standalone updater for the OctoWoW client: game files, mods,
tweaks, texture packs and addons. Standard library, optional 'certifi' for
TLS; Python 3.10+.

Derived from Octo Updater by rebasedkon (see NOTICE), and reworked around one
rule: **EqUpdater manages what EqUpdater installed, or what the user asked it
to manage. It may inspect everything else, but it must not silently replace
it.**

The machinery that enforces that rule does not live in this file. Every
decision about whether a component may be touched comes from
`equpdater.planner.plan()`, which is a pure function over facts gathered
here; the version ordering behind it is `equpdater.versions`, the commit
ancestry is `equpdater.gitcompare`, and the local-modification check is
`equpdater.hashing`. This file gathers facts, draws them, and carries out
whatever the planner authorised - it does not decide.

That separation is the point. In the updater this grew from, the mods list,
the addons list, the apply worker and Update All each decided independently
what was "out of date", and four implementations of one judgement drifted
...
# puts the game. All three now come from equpdater.branding, which knows the
# platform conventions - the inherited version wrote settings beside the
# executable on Linux, which breaks the moment the app is installed anywhere
# read-only.
...
# load_config / save_config / update_config / ensure_dir are imported from
# equpdater.config. Same names, same contract - atomic writes and one lock
# shared by the UI thread and every worker - so the several hundred call
# sites below are unchanged. What is new is underneath them: records carry
# provenance (who installed this, from where, and what did it look like) and
# a first launch migrates Octo Updater's settings across.
...
# EqUpdaterApp._poll) and renders each line. This keeps all Tk access on the
...
# This product's own releases. Octo Updater's version line stopped being
# comparable with EqUpdater's the moment the schema and the safety model
# changed, so the daily check watches this repository; upstream is credited
# in the About box and in NOTICE rather than polled.
UPDATER_REPO      = branding.PROJECT_REPO
...
def backup_component(path: str, label: str) -> str | None:
    """Copy a file or folder aside before something destructive happens to it.

    Only ever called from an explicit force/replace path. Routine updates do
    not back anything up: an updater that copied every addon on every update
    would fill the disk with noise nobody reads, and the noise is what makes
    the one useful backup impossible to find.

    Returns where it went, or None. **A failed backup is not fatal** - it is
    reported and the caller decides. Refusing to proceed because a copy could
    not be made would leave somebody unable to fix a broken addon on a full
    disk."""
    ...
    try:
        ...
        log(f"  Could not back up {os.path.basename(path)}: {e}")
        return None


def mod_supports_update_check(mod: dict) -> bool:
    """Whether this mod's source can be asked what the latest version is.

    A pinned direct file has no release feed, so there is nothing to compare
    against and nothing to update to. That is a fact about the source, not a
    judgement about the files."""
    return mod["source"]["kind"] not in ("direct_file", "direct_tar")


def mod_files_on_disk(mod: dict, client_dir: str) -> bool:
    """Whether this mod's *registry-declared* files are all present.

    Distinct from `mod_installed_files_present`, which asks about the files a
    previous install recorded. This one answers "is something that looks like
    this mod already here" for a client EqUpdater has never touched -- the
    first-run discovery question."""
    files = mod.get("installed_files") or []
    return bool(files) and all(
        os.path.exists(os.path.join(client_dir, f)) for f in files)


def mod_component(mod: dict, state: dict, live: dict | None,
                  client_dir: str = "") -> Component:
    """Gather everything the planner needs about one DLL mod.

    **This function decides nothing.** It reads config, disk and the cached
    release lookup, and hands the facts to `plan()`. Every consumer -- the
    row, the badge, Apply, Update All -- goes through here, so they cannot
    disagree about what a mod's state is.

    Two subtleties worth naming:

    `managed` defaults to whether a version was ever recorded, because a
    record written by the inherited schema has no `managed` key and a
    recorded version is the only evidence that updater installed it. Files
    found on disk with no record at all are *not* managed: that is somebody
    else's DLL, or one from another updater, and EqUpdater's job there is to
    show it, not to replace it with its own preferred build.

    `lookup_failed` is true whenever a version check was wanted and did not
    produce an answer. It has to be distinguishable from "no update", because
    an unreachable GitHub must never be the reason files get replaced."""
    installed_ver = state.get("installed_version")
    recorded = bool(installed_ver) or bool(state.get("installed_files"))
    managed = bool(state.get("managed", recorded))
    files = state.get("installed_files") or mod.get("installed_files") or []

    present = bool(files) and all(
        os.path.exists(os.path.join(client_dir, f)) for f in files) \
        if client_dir else True

    # Something on disk that no record claims: discovered, never owned.
    installed = recorded or (bool(client_dir) and mod_files_on_disk(mod, client_dir))

    remote_ver = (live or {}).get("latest_version")
    checkable = mod_supports_update_check(mod)
    lookup_failed = bool(checkable and managed and installed
                         and not remote_ver)

    recorded_hash = state.get("content_hash")
    current_hash = (files_hash(client_dir, files)
                    if recorded_hash and client_dir and present else None)

    return Component(
        id=mod["id"], kind="mod", name=mod.get("name") or mod["id"],
        installed=installed, managed=managed,
        enabled=bool(state.get("enabled", False)),
        ignore_updates=bool(state.get("ignore_updates", False)),
        files_present=present,
        installed_version=installed_ver,
        # A pinned source offers no version to compare with; saying so is
        # what makes the row read "version not tracked" rather than pretending
        # the pinned copy is current.
        remote_version=remote_ver if checkable else installed_ver,
...
def mod_plan(mod: dict, state: dict, live: dict | None,
             client_dir: str = "") -> Plan:
...
def _same_git_repo(a, b) -> bool:
    ...
    Delegates to gitcompare.same_repo so the UI, the planner and this file
    agree on what "the same repository" means. They did not before: a
    difference the planner would call a source change could read here as the
    same repo, or the other way about, and the two answers drove different
    buttons."""
    return same_repo(a, b)


def addon_is_managed(saved: dict | None) -> bool:
    """Whether EqUpdater owns this addon.

    The v2 schema says so outright. A record inherited from Octo Updater does
    not have the field, and for those the evidence is a recorded commit sha
    *and* the repository it came from -- which together mean that updater
    installed it. A folder with neither is on disk because somebody put it
    there, and it stays theirs until they say otherwise."""
    if not saved:
        return False
    if "managed" in saved:
        return bool(saved["managed"])
...
def addon_component(folder: str, saved: dict | None, *,
                    ...
                    ancestry_result=None, lookup_failed: bool = False,
                    error=None, installed_version=None) -> Component:
    """Gather one addon's facts for the planner.

    The one thing worth spelling out: `installed_source` is the repository
    the files *came from*, and `configured_source` is the one the catalogue
    currently recommends. Keeping them apart is what turns "this addon is out
    of date" back into the two different questions it always was -- is there
    a newer commit on the source you installed from, and separately, would
    you like to switch to a different fork."""
    saved = saved or {}
    managed = addon_is_managed(saved)
    return Component(
        id=folder, kind="addon", name=folder,
        installed=installed, managed=managed, enabled=True,
        ignore_updates=bool(saved.get("ignore_updates", False)),
        files_present=installed,
        installed_sha=saved.get("sha"), remote_sha=remote_sha,
        ancestry=ancestry_result,
        installed_source=saved.get("git"),
        configured_source=configured_source,
        custom_source=bool(saved.get("custom_source",
                                     saved.get("custom", False))),
        recorded_hash=saved.get("content_hash"), current_hash=current_hash,
        installed_version=installed_version,
        lookup_failed=lookup_failed, error=error,
    )


#: Planner status -> the short machine string the addon rows and badges use.
#: One mapping, so a status cannot mean one thing to the badge and another to
#: the row -- which is what "outOfDate" managed to do for four situations at
#: once.
ADDON_STATUS_NAMES = {
    Status.UPDATE_AVAILABLE: "updateAvailable",
    Status.UP_TO_DATE:       "upToDate",
    Status.LOCAL_NEWER:      "localNewer",
    Status.UNMANAGED:        "unmanaged",
    Status.MODIFIED:         "modified",
    Status.DIFFERENT_SOURCE: "differentSource",
    Status.DIVERGED:         "diverged",
    Status.UNVERIFIABLE:     "unverifiable",
    Status.IGNORED:          "ignored",
    Status.ERROR:            "invalid",
    Status.MISSING_FILES:    "invalid",
    Status.NOT_INSTALLED:    "available",
...
_MARK = branding.PFUI_MARK_PREFIX
_PFUI_MARK_BEGIN = f"-- {_MARK}_DEFAULT_PROFILE_BEGIN"
_PFUI_MARK_END   = f"-- {_MARK}_DEFAULT_PROFILE_END"
_PFUI_CHAT_BEGIN = f"-- {_MARK}_CHAT_SKIP_BEGIN"
_PFUI_CHAT_END   = f"-- {_MARK}_CHAT_SKIP_END"

# Strips an injected block whichever marker pair it carries -- **including
# the previous product's**. A profile written by Octo Updater has
# OCTO_UPDATER_ markers in it, and a stripper that only knew the new spelling
# would leave that block behind and add a second one beside it, doubling the
# injected settings every launch.
_PFUI_PREFIXES = "|".join([_MARK, *branding.PFUI_LEGACY_MARK_PREFIXES])
_PFUI_STRIP_RE = (r"[ \t]*-- (?:" + _PFUI_PREFIXES +
                  r")_[A-Z_]+?_BEGIN.*?-- (?:" + _PFUI_PREFIXES +
                  r")_[A-Z_]+?_END\n?")
...
class EqUpdaterApp(tk.Tk):
    def __init__(self):
        ...
        # **The inheritance step**, run before anything reads a setting.
        # Finds Octo Updater's config if this machine has one, copies it
        # aside, carries the compatible settings across and stamps the new
        # schema so it never runs twice. A fresh machine gets an empty
        # stamped config and nothing else happens.
        #
        # Records come across as *provenance*: what the old updater recorded
        # as installed becomes managed-and-inherited, and anything it only
        # ever saw becomes unmanaged. An old record is evidence that updater
        # installed something; it is not evidence that the files on disk are
        # still what it installed, and the migration is careful not to
        # confuse the two. See equpdater.config.migrate_records.
        try:
            self._cfg = bootstrap_config(
                client_dir=(load_config().get("out_dir") or ""),
                addon_dir_for=lambda folder: os.path.join(
                    addons_path(load_config().get("out_dir") or ""), folder))
            if self._cfg.get("migrated_from"):
                log("")
                log("Imported settings from %s."
                    % self._cfg["migrated_from"]["product"])
                if self._cfg["migrated_from"].get("backup"):
                    log("  A copy of the old configuration is kept at %s."
                        % self._cfg["migrated_from"]["backup"])
        except Exception as e:
            # A failed migration must not stop the app starting. No stamp is
            # written, so the next launch tries again rather than leaving the
            # user half-migrated with no way back.
            log(f"Could not import previous settings: {e}", "err")
        ...
        # (and stamp app_version) before verifying. It runs
    ...
    def _set_tooltip(self, widget, text: str):
        """A tooltip whose text can change without stacking another handler.

        `_add_tooltip` binds with add="+", which is right for a label built
        once and never touched again. Rows are different: they are restyled
        on every refresh, and calling _add_tooltip each time leaves one live
        handler per refresh, each showing the text that was current when it
        was bound. Here the text lives on the widget and the binding happens
        once."""
        widget._tip_text = text
        if getattr(widget, "_tip_bound", False):
            return
        widget._tip_bound = True
        self._add_tooltip_dynamic(widget)

    def _add_tooltip_dynamic(self, widget):
        state = {"win": None}
    ...
    def _on_hdr_click(self, event):
        ...
        if name == "gear":
            ...
            self._open_url(branding.PROJECT_URL)
    ...
    def _new_version_installed(self):
        ...
        The config key ``app_version`` records the updater version
        ...
        block below."""
        cfg = load_config()
        if cfg.get("app_version") == UPDATER_VERSION:
            return

        if not self._first_run:
            from_ver = _parse_version(cfg.get("app_version", ""))
            if from_ver < (1, 3):
                self._migrate_1_3()

        self._cfg = update_config(
            lambda c: c.__setitem__("app_version", UPDATER_VERSION))
    ...
    def _render_mod_rows(self):
        ...
        for mod in mods_sorted:
            ...
            # Right-click is the deliberate, out-of-the-way gesture for the
            # deliberate, destructive action: install the source's version
            # over whatever is here, downgrade included. Never reachable by
            # a stray left-click on a label that reads "local newer".
            update_label.bind("<Button-3>",
                              lambda e, m=mid: self._mod_force_replace(m))
    ...
    def _mod_plans(self) -> dict:
        """Every mod's decision, keyed by id.

        The rows, the badge, Apply and Update All all read this. Before the
        planner existed each computed its own answer and they drifted: a row
        could offer an update the worker then skipped, and Update All could
        sweep a mod no row had ever offered."""
        ...
        for mod in MODS_REGISTRY:
            ...
            live = next((m for m in self._mods_state
                         if m["id"] == mod["id"]), None)
            out[mod["id"]] = mod_plan(mod, state, live, client)
        return out

    def _count_mod_updates(self) -> int:
        """The badge: mods that can be safely updated, and nothing else.

        A local-newer, modified or unmanaged mod is not a pending task and
        counting it as one is how a badge trains people to click Update All
        without reading."""
        return sum(1 for p in self._mod_plans().values() if p.will_update)
    ...
    #: Gold means "this will act"; olive and dim mean "this is information".
    ...
    def _style_mod_action_label(self, lbl, mod, state, live, plan_=None):
        """Drive the per-row action label from the mod's plan.

        Every state the planner can reach has an entry here or is hidden.
        The important half is what is *not* offered: local-newer, modified
        and unmanaged mods get a word and a tooltip, not a button, because
        there is nothing safe for a click to do."""
        if state.get("error"):
        ...
        client = self._game_path.get().strip()
        p = plan_ or mod_plan(mod, state, live, client)
        entry = self._MOD_ACTIONS.get(p.status)
        if not entry:
            lbl.pack_forget()
            return
        ...
        lbl._base = colour
        # An informational label does not brighten under the cursor. Looking
        # interactive is a promise, and there is nothing here to click.
        lbl._hover = {"act": C_GOLD_LT, "adopt": C_GOLD_LT}.get(mode, colour)
        lbl._mode = mode
        lbl.configure(text=text, fg=colour,
                      cursor="hand2" if mode in ("act", "adopt") else "arrow")
        self._set_tooltip(lbl, tip)
        lbl.pack(side="right", padx=(self._px(2), self._px(8)))

    def _mod_action_click(self, mod_id: str):
        """What the row's label does, which depends on what it says.

        An informational label does nothing at all when clicked. That is
        deliberate: a label that reads "local newer" and then downgrades the
        mod on click would be the original bug wearing a warning label."""
        refs = self._mod_row_vars.get(mod_id, {})
        lbl = refs.get("update_label")
        mode = getattr(lbl, "_mode", "act") if lbl else "act"
        if mode == "adopt":
            self._adopt_mod(mod_id)
        elif mode == "act":
            self._update_mod(mod_id)

    def _adopt_mod(self, mod_id: str):
        """Hand a discovered DLL over to EqUpdater, with consent.

        Adoption records ownership and a fingerprint of the files *as they
        are* -- it does not install anything. The user keeps the exact build
        they had; EqUpdater simply gains permission to offer them updates to
        it from here on, and can tell if those files change afterwards."""
        from tkinter import messagebox
        ...
        def _merge(c):
            mods = c.setdefault("mods", {})
            rec = dict(mods.get(mod_id) or {})
            rec.update(new_mod_record(
                managed=True, adopted=True,
                installed_version=rec.get("installed_version"),
                installed_files=files,
                content_hash=files_hash(client, files),
                enabled=True))
            rec["ignore_updates"] = bool(
                (mods.get(mod_id) or {}).get("ignore_updates", False))
            mods[mod_id] = rec
    ...
    def _maybe_install_essential_mods(self):
        ...
        **Discovery happens first, and it is the part that changed.** The
        inherited version installed its own preferred build of every
        essential mod the moment a game folder became usable -- including
        over a client that already had them, possibly at newer versions,
        possibly customised. A first run against somebody's existing install
        is not a fresh install and must not be treated as one.

        So anything already on disk is *recorded as found* rather than
        replaced: unmanaged, visible on the MODS page, and offered for
        adoption. Only the genuinely absent ones are installed."""
        ...
        if discovered:
            self._log_line(
                "\nFound %d mod(s) already installed here: %s.\n"
                "Left untouched — open the MODS tab to have %s manage "
                "them.\n" % (len(discovered), ", ".join(sorted(discovered)),
                             branding.APP_NAME), "acct")
    ...
    def _discover_existing_mods(self, client_dir: str) -> set:
        ...
        Recorded as unmanaged with a fingerprint of the files as found, so
        the MODS page can show what is there, name it, and offer to manage
        it -- and so that if the user later adopts it, EqUpdater can tell
        whether the files changed in between.

        No version is claimed. The files were not installed by this updater
        and their release cannot be inferred from a DLL on disk; the row says
        "version not tracked" rather than guessing, because a guessed version
        is what a comparison would then act on."""
        found = {}
        for mod in MODS_REGISTRY:
            files = mod.get("installed_files") or []
            if not files or not mod_files_on_disk(mod, client_dir):
                continue
            rec = new_mod_record(
                managed=False, enabled=True,
                installed_files=files,
                content_hash=files_hash(client_dir, files),
        ...
        if found:
            def _merge(c):
                ...
                for mid, rec in found.items():
                    mods.setdefault(mid, rec)
            self._cfg = update_config(_merge)
        return set(found)

    def _apply_mods_worker(self, client_dir: str, only_mod_id: str | None = None,
                           force: bool = False):
        """Install, update and remove mods according to the plan.

        `force` is the only way a downgrade or a replacement of modified
        files can ever happen, and nothing sets it except an explicit user
        action that said so in a dialog first. Update All and every ordinary
        Apply leave it false."""
        ...
        for mod in ordered:
            ...
            # **The decision, and the only place it is made.** Whether an
            # installed mod gets replaced is the planner's answer, not this
            # worker's opinion -- so the row, the badge, Update All and this
            # loop cannot disagree, and the safety invariants hold here
            # because they hold in one function that is tested directly.
            #
            # The old line here was `latest_ver != installed_ver`, which is
            # how V90 became V89.
            mod_state = dict(state)
            mod_state.update({"enabled": enabled, "ignore_updates": ignore_upd})
            decision = mod_plan(
                mod, mod_state,
                {"latest_version": latest_ver} if latest_ver else None,
                client_dir)

            needs_update = enabled and is_installed and (
                decision.will_update or (force and latest_ver is not None))

            # Anything that looked like it wanted updating and deliberately
            # was not gets said out loud. A silent skip is indistinguishable
            # from a bug, and from the outside it looks exactly like the
            # updater not working.
            if (enabled and is_installed and not needs_update
                    and not needs_uninstall and decision.skipped
                    and decision.action not in (Action.SKIP_UP_TO_DATE,
            ...
            try:
                if needs_install:
                    ...
                    # The record stays, saying we no longer own anything
                    # here. Dropping the key would make the mod look
                    # never-seen -- and a never-seen mod whose files are
                    # still on disk is exactly the unmanaged case, which
                    # would then be offered for adoption seconds after the
                    # user asked for it to be removed.
                    mods_cfg[mid] = new_mod_record(managed=False, enabled=False)
        ...
        def worker():
            ...
            def apply():
                ...
                for row in self._mpq_installed_rows:
                    state = results.get(row["file"])
                    if state:
                        row["state"], row["update"] = state
        ...
        def worker():
            ...
            if ap and os.path.isdir(ap):
                for name in sorted(os.listdir(ap)):
                    ...
                    # the user says so: a folder called Questie matching a
                    # Questie entry is a coincidence of naming, not a claim
                    # of ownership over whatever is in it.
                    suggested = override or (avail or {}).get("git")
                    rec["suggested_git"] = suggested
                    rec["managed"] = addon_is_managed(saved)

                    if not rec["managed"]:
                        # **Found, not owned.** The inherited updater matched
                        # this folder against the catalogue and offered to
                        # "update" it -- which meant deleting somebody's
                    ...
                    # Managed: everything below asks about the source the
                    # files actually came from, never about the catalogue's
                    ...
                    if remote_checks:
                        ...
                        remote = addon_cached_sha(
                            rec["git"], rec["branch"], rec["ref"])
                        if remote is None:
                            # No cached answer and no network wanted: treat
                            # it as current rather than guessing, and do not
                            # record a failure the user would have to clear.
                            remote = saved.get("sha")
                    if remote is None:
                        lookup_failed = True

                    # Which came first. A different sha is an observation;
                    # this is the question. Skipped when the answer cannot
                    # change anything -- same sha, or nothing to compare.
                    anc = None
                    if (remote and saved.get("sha")
                            and remote != saved.get("sha") and remote_checks):
                        anc = ancestry(rec["git"], saved["sha"], remote,
                                       fetch=_api_json)
                    elif remote and remote == saved.get("sha"):
                        anc = Ancestry.IDENTICAL

                    comp = addon_component(
                        name, saved, installed=True, remote_sha=remote,
                        configured_source=suggested,
                        current_hash=(folder_hash(dirp)
                                      if saved.get("content_hash") else None),
                        ancestry_result=anc, lookup_failed=lookup_failed,
                        installed_version=rec["toc"].get("Version"))
                    decision = plan(comp)
                    rec["plan"] = decision
                    rec["status"] = ADDON_STATUS_NAMES.get(
        ...
        def worker():
            for rec in recs:
                ...
                try:
                    ...
                    # Provenance, written the moment the files land: we
                    # installed this, from this repository, at this commit,
                    # and this is the fingerprint of what arrived. The
                    # fingerprint is what lets a later run tell "the user
                    # edited this" from "the user did not", which the commit
                    # sha alone never could.
                    record = new_addon_record(
                        managed=True, git=rec["git"],
                        branch=rec.get("branch"), ref=rec.get("ref"),
                        sha=sha,
                        content_hash=folder_hash(
                            os.path.join(addons_path(client), rec["folder"])),
                        custom_source=bool(rec.get("custom")))
                    old_rec = load_config().get("addons", {}).get(
                        rec["folder"]) or {}
                    record["ignore_updates"] = bool(
                        old_rec.get("ignore_updates", False))
    ...
    def _addon_update_all(self):
        ...
        source it was installed from. Nothing else.

        The old version swept `status == "outOfDate"`, which covered four
        unrelated situations -- a newer remote, a manually installed addon, a
        locally edited one, and a catalogue preferring a different fork -- and
        replaced all four."""
    ...
    def _render_addons(self):
        ...
        # **Grouped by ownership, because ownership is the thing to know.**
        # One "INSTALLED" list mixed addons EqUpdater looks after with addons
        # it merely found, and the rows gave no clue which was which - so an
        # Update button on one and an Update button on the other looked like
        # the same button doing the same thing. They were not.
        every = keep(st["addons"].values())
        managed = [r for r in every if r.get("status") != "unmanaged"]
        unmanaged = [r for r in every if r.get("status") == "unmanaged"]
    ...
    def _addon_row(self, rec: dict):
        ...
        if installed:
            ...
            # **Stop managing**, next to Remove and deliberately unlike it:
            # one gives up ownership, the other deletes files. Offered only
            ...
            if rec.get("status") not in ("unmanaged", "available", None):
                ...
                rel.bind("<Button-1>",
                         lambda e, n=rec["folder"]: self._addon_release(n))
            ...
            # _addon_switch_source.
            suggested = rec.get("suggested_git")
            if (suggested and rec.get("git")
                ...
                sw.bind("<Button-1>",
                        lambda e, n=rec["folder"], g=suggested:
                        self._addon_switch_source(n, g))
        ...
        self._addon_row_status(row, rec, installed, warnings)
    ...
    #: Addon status -> (label, colour, tooltip). Anything that is not an
    ...
    def _addon_row_status(self, row, rec, installed, warnings):
        """The right-hand end of an addon row: what it is, and what may be
        done about it.

        Two things here are buttons - Update on a proven newer commit, and
        Manage on something EqUpdater found but does not own. Everything else
        is a statement of fact with a tooltip explaining it, because there is
        nothing safe for a click to do."""
        ...
        if status == "unmanaged":
            # Found on disk. The offer is to *manage* it - which changes
            # nothing now - and never to "update" it, because nothing here
        ...
        if status == "updateAvailable" and installed:
            ...
            if rec.get("installed_sha") and rec.get("remote_sha"):
                self._set_tooltip(upd, "%s → %s"
                                  % (short_sha(rec["installed_sha"]),
                                     short_sha(rec["remote_sha"])))
            return
        ...
        if entry:
            ...
            if status in ("modified", "localNewer", "diverged"):
                ...
                rep.bind("<Button-1>",
                         lambda e, r=rec: self._addon_force_replace(r))
    ...
    def _addon_adopt(self, rec):
        """Hand a manually installed addon over to EqUpdater, with consent.

        **This installs nothing.** It records that the user wants EqUpdater
        to look after the folder, pins the source it should track, and
        fingerprints the files exactly as they are. The addon on disk is
        untouched; what changes is that EqUpdater may now offer updates for
        it, and can tell afterwards whether those files changed.

        No commit sha is claimed, because none is known -- whatever is in
        that folder did not necessarily come from the repository the
        catalogue suggests, and inventing a sha would let the next run
        "update" a fork into a different fork. With no sha the planner
        answers "unable to verify" until the user explicitly installs from
        the source once, which is the honest sequence."""
        from tkinter import messagebox
        folder = rec["folder"]
        git = rec.get("suggested_git") or rec.get("git")
        if not git:
            return
        if not messagebox.askyesno(
        ...
        client = self._game_path.get().strip()
        record = new_addon_record(
            managed=True, adopted=True, git=git,
            content_hash=folder_hash(
                os.path.join(addons_path(client), folder)) if client else None)
        update_config(lambda c, f=folder, r=record:
                      c.setdefault("addons", {}).__setitem__(f, r))
        self._log_line("\n%s is now managed by %s, tracking %s.\n"
                       "Its files were not changed.\n"
                       % (folder, branding.APP_NAME, git), "acct")
        self._addons_verified_ts = 0.0
        self._addons_verify(force=False)

    def _addon_release(self, folder: str):
        """Stop managing an addon. The files stay exactly where they are.

        Releasing ownership is not uninstalling: the row moves to INSTALLED
        MANUALLY and every automatic flow stops considering it. This is the
        escape hatch for somebody who wants to look after one addon by hand
        without turning the whole updater off."""
        rec = (load_config().get("addons", {}) or {}).get(folder)
        if not rec:
            return

        def _merge(c):
            r = c.setdefault("addons", {}).setdefault(folder, {})
            r["managed"] = False
            r["installed_by"] = None
        update_config(_merge)
        self._log_line("\n%s is no longer managed by %s; its files are "
                       "untouched.\n" % (folder, branding.APP_NAME), "acct")
        self._addons_verified_ts = 0.0
        self._addons_verify(force=False)

    def _addon_switch_source(self, folder: str, new_git: str):
        """Point a managed addon at a different repository, deliberately.

        Recorded as a *custom* source, which means the catalogue will not
        quietly point it somewhere else again later. The installed sha is
        dropped: it belongs to the old repository's history and comparing it
        against the new one's commits would be meaningless. The addon then
        reads "unable to verify" until it is installed from the new source
        once -- which is accurate, and better than a confident wrong answer."""
        from tkinter import messagebox
        ...
        def _merge(c):
            ...
            r["ref"] = None
            r["sha"] = None
            r["custom_source"] = True
            r["managed"] = True
        update_config(_merge)
        self._log_line("\n%s now tracks %s.\n" % (folder, new_git), "acct")
        self._addons_verified_ts = 0.0
        self._addons_verify(force=True)

    def _refresh_addons_badge(self):
        """Addons that can be safely updated. An unmanaged, modified or
        diverged addon is not a pending task; counting it as one is how a
        badge teaches people to press Update All without reading it."""
    ...
    def _refresh_addons_footer(self):
        ...
        if st["state"] == "verifying" or self._addons_busy:
            ...
            # true, and saying it anyway is how a warning gets ignored.
            held = sum(1 for r in st["addons"].values()
                       if r["status"] in ("localNewer", "modified",
    ...
    def _addon_plans(self) -> list:
        ...
        Addon state is expensive to establish -- a commit lookup and possibly
        an ancestry call per addon -- so the verify records each decision on
        the row and this reads them back rather than asking again."""
        out = []
        for rec in self._addons_status["addons"].values():
            p = rec.get("plan")
            if p is not None:
                out.append(p)
        return out

    def _all_plans(self) -> list:
        """Everything EqUpdater has an opinion about, mods and addons."""
        return list(self._mod_plans().values()) + self._addon_plans()

    def _update_all_preview(self, plans) -> bool:
        """Show what Update All will and will not do; True to go ahead.

        Shown only when something is being held back. A run where everything
        is ordinary is one click, which is the point of the button -- and a
        confirmation nobody ever needs is a confirmation everybody learns to
        dismiss, which is how the one that mattered gets dismissed too."""
        from tkinter import messagebox
        doing = updatable(plans)
        held = skipped_notably(plans)
        if not held:
            return True
    ...
    def _update_all(self):
        ...
        It does **not** mean "make this installation match the catalogue".
        Mods run through the normal Apply worker, which asks the planner per
        mod; when it finishes, `_update_all_chain` hands over to the addons.
        Both halves consult the same decisions the preview just showed, so
        what runs is what was agreed to."""
        if not self._all_ready:
            return
        out = self._game_path.get().strip()
        if not out:
        ...
        if not self._update_all_preview(plans):
            self._refresh_update_all_btn()
            return

        mods = self._mod_updates_count
        addons = self._addon_updates_count
        self._log_line("\nUpdating everything: %d mod(s), %d addon(s)...\n"
                       % (mods, addons), "acct")

        # Every hold-back, in the log, with its reason. A skip nobody is told
        # about is indistinguishable from the updater not working -- and the
        # silence is what let the old behaviour hide a downgrade.
        for p in skipped_notably(plans):
            self._log_line("  \u2022 %s \u2014 %s\n"
                           % (p.component_id, p.reason), "")
    ...
    def _addon_force_replace(self, rec):
        """Overwrite a managed addon with the latest from its source, after
        saying plainly what will be lost.

        This is the way out of "modified locally", "local version newer" and
        "different revision" - the three states that hold an update back. It
        exists precisely so those states can be strict: a user who genuinely
        wants the remote copy has one button, and everybody else is not
        silently given it.

        The folder is copied into the backups directory first, and where it
        went is printed in the log, because "replace my changes" is only a
        reasonable thing to agree to if the changes are recoverable."""
        from tkinter import messagebox
        folder = rec["folder"]
        client = self._game_path.get().strip()
        if not client or not rec.get("git"):
            return
        ...
        dest = backup_component(
            os.path.join(addons_path(client), folder), "addons")
        if dest:
            self._log_line("\nBacked up %s to %s\n" % (folder, dest), "acct")
    ...
    def _mod_force_replace(self, mod_id: str):
        ...
        The V90 -> V89 case is the reason it is worded the way it is: this is
        the *only* path that can install an older release over a newer one,
        and it says so before it does."""
        from tkinter import messagebox
        mod = next((m for m in MODS_REGISTRY if m["id"] == mod_id), None)
        client = self._game_path.get().strip()
        if not mod or not client:
            return
        ...
        for rel in (state.get("installed_files")
                    or mod.get("installed_files") or []):
            backup_component(os.path.join(client, rel), "mods")
